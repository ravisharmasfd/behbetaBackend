    ...
    if (type == 1) {
      ...
      if (sendAtSMS) {
        ...
        const res = await sendSms(
          ...
          `You have received an order from ${req.user?.businessName} for an amount of ${amount} BHD. Pay using:${url}`,
        );
      }
...
exports.generatePaymentSeasonId = async (req, res, next) => {
  try {
    ...
    const season = await createPayment(
      invoice?.amount,
      "BHD",
      invoice?.user_id?.apiKey,
      invoice.user_id?.businessName,
      id,
      "invoice",
    );
    res.send({
...
exports.cancelInvoice = async (req, res, next) => {
  try {
    ...
    // Find and delete the invoice
    const canceledInvoice = await Invoice.findOneAndUpdate(
      { _id: id, user_id: req.user._id },
      { $set: { status: 3 } },
    );
    ...
    res.send({
      message: "Invoice is canceled successfully",
