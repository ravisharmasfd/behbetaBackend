...
cron.schedule("0 10 * * *", async () => {
  ...
  try {
    ...
    invoices.forEach((inv) => {
      ...
      const newDate = moment(body.invoice_start_date).add(
        repeat_every,
        frequencyUnit,
      );
      let nextInvoice = new Invoice({
        ...body,
        invoice_start_date: newDate,
      });
      newInvoicesPromise.push(nextInvoice.save());
      if (body.sendAtMail) {
        sendSmsPromise.push(
          sendEmail(body.email, url, body.amount, body?.user_id?.businessName),
        );
      }
      if (body.sendAtSMS) {
        sendSmsPromise(
          sendSms(
            body?.country_code + body?.mobile_no,
            `You have received an order from ${body?.user_id?.businessName} for an amount of ${body?.amount} BHD. Pay using:${url}`,
          ),
        );
      }
    });
